function plot_results(agent,nsteps,fmode,outImages)
    ...
    disp(strcat('No. new vaccinates = ',num2str(IT_STATS.div_r(N_IT+1))))
    disp(strcat('No. new infected = ',num2str(IT_STATS.div_f(N_IT+1))))
    disp(strcat('No. agents migrating = ',num2str(IT_STATS.mig(N_IT+1))))
    disp(strcat('No. vaccinated dying = ',num2str(IT_STATS.died_r(N_IT+1))))
    disp(strcat('No. infected dying = ',num2str(IT_STATS.died_f(N_IT+1))))
    disp(strcat('No. vaccinated eaten = ',num2str(IT_STATS.eaten(N_IT+1))))
    ...
    if (fmode==false) || (N_IT==nsteps) || ((fmode==true) && (rem(N_IT , CONTROL_DATA.fmode_display_every)==0))
        ...
        col{1}='r-';                   %set up colours that will represent different cell types red for vaccinated, blue for infected
        col{2}='b-';
        ...
        subplot(3,1,1),title('No. live vaccinated');
        subplot(3,1,2),title('No. live infected');
        ...
        for cn=1:length(agent)                          %cycle through each agent in turn
            if typ(cn)>0                                %only plot live agents
                ...
                if isa(agent{cn},'vaccinated')              %choose plot colour depending on agent type
