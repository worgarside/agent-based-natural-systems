...
fprintf('Infected deaths: %i\n', IT_STATS.dead_infected(N_IT+1))
fprintf('Infections:      %i\n', IT_STATS.infections(N_IT+1))
...
if (fmode==false) || (N_IT==nsteps) || ((fmode==true) && (rem(N_IT , CONTROL_DATA.fmode_display_every)==0))
    ...
    n = vacc_count(N_IT+1)+inf_count(N_IT+1);             %current agent number
    f2=figure(2);
    set(f2,'Units','Normalized');
    set(f2,'Position',[0.5 0.5 0.45 0.4]);
    
    subplot(3,1,1), cla
    subplot(3,1,1), plot((1:N_IT+1), vacc_count(1:N_IT+1), col{1});
    subplot(3,1,1), axis([0 nsteps 0 1.1*max(vacc_count)]);
    subplot(3,1,2), cla
    subplot(3,1,2), plot((1:N_IT+1), inf_count(1:N_IT+1), col{2});
    subplot(3,1,2), axis([0 nsteps 0 1.1*max(inf_count)]);
    subplot(3,1,3), cla
    subplot(3,1,3), plot((1:N_IT+1), 1, 'm-');
