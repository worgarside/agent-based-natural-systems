...
%migration functions for class INFECTED
%agt=infected object
%cn - current agent number

%SUMMARY OF INFECTED MIGRATE RULE
%If a infected has not eaten, it will pick a random migration direction
...
mig=0;
cnt=1;
dir=rand*2*pi;              %infected has been unable to find food, so chooses a random direction to move in
while mig==0&&cnt<=8        %infected has up to 8 attempts to migrate (without leaving the edge of the model)
    npos(1)=pos(1)+spd*cos(dir);        %new x co-ordinate
    npos(2)=pos(2)+spd*sin(dir);        %new y co-ordinate
    if npos(1)<bm&&npos(2)<bm&&npos(1)>=1&&npos(2)>=1   %check that infected has not left edge of model - correct if so.
       mig=1;
    end
    cnt=cnt+1;
