...
[agent]=create_agents(nr,nf);       %create nr vaccinated and nf infected agents and places them in a cell array called 'agents'
...
for n_it=1:nsteps                   %the main execution loop
    ...
    if fmode == true                                       % if fastmode is used ...
        ...
        if IT_STATS.tot_r(n_it) == 0             %fastmode convergence - all vaccinateds eaten - all infectedes will now die
            disp('Fast mode convergence criteria satisfied - no vaccinateds left alive! > ')
            break
        end
        if IT_STATS.tot_f(n_it) == 0             %fastmode convergence - all infectedes starved - vaccinateds will now proliferate unchecked until all vegitation is eaten
